...
GETTEXT_INSTALLDIR=${PREFIX}/tools/gettext/${GETTEXT_VERSION}
...
cp ${SCRIPT_DIR}/${SCRIPT_NAME} \
   ${GETTEXT_INSTALLDIR}/build-gettext-${GETTEXT_VERSION}.sh

# Write module 

mkdir -p ${PREFIX}/modules/gettext
cat > ${PREFIX}/modules/gettext/${GETTEXT_VERSION} << EOF
#%Module1.0#####################################################################
##
## null modulefile
##
