...
GCC_INSTALLDIR=${PREFIX}/compilers/gcc/${GCC_VERSION}
...
cp ${SCRIPT_DIR}/${SCRIPT_NAME} \
   ${GCC_INSTALLDIR}/build-gcc-${GCC_VERSION}.sh

# Write module 

mkdir -p ${PREFIX}/modules/gcc

cat > ${PREFIX}/modules/gcc/${GCC_VERSION} << EOF
#%Module1.0#####################################################################
##
## null modulefile
##
...
setenv ESE_COMPILER gcc${GCC_VERSION}
