...
FLEX_INSTALLDIR=${PREFIX}/tools/ese-flex/${FLEX_VERSION}
...
cp ${SCRIPT_DIR}/${SCRIPT_NAME} \
   ${FLEX_INSTALLDIR}/build-ese-flex-${FLEX_VERSION}.sh

# Write module 

mkdir -p ${PREFIX}/modules/ese-flex
cat > ${PREFIX}/modules/ese-flex/${FLEX_VERSION} << EOF
#%Module1.0#####################################################################
##
## null modulefile
##
