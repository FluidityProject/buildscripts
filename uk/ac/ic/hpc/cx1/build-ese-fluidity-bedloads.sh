...
FLUIDITY_GIT_URL=https://github.com/jmn114/fluidity.git
FLUIDITY_GIT_BRANCH=bedload
FLUIDITY_VERSION=`git ls-remote ${FLUIDITY_GIT_URL} ${FLUIDITY_GIT_BRANCH} | awk '{print substr($1,1,6);}'`
...
${FLUIDITY_SOURCEDIR}/configure --prefix=${FLUIDITY_INSTALLDIR} \
      --enable-2d-adaptivity --enable-debugging

# Build and install
make makefiles && make && make fltools && make install
