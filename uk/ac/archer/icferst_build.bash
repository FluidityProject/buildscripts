...
export BUILDSCRIPTS=/work/e637/e637/akampits/buildscripts/uk/ac/archer

# EDIT REQUIRED: Set the following to your own Fluidity directory on
# /work
export FLUIDITYDIR=/work/e637/e637/akampits/fluidity
export ICDIR=/work/e637/e637/akampits/multifluids_icferst
 
# Copy the PrgEnv-fluidity module to the Fluidity directory so
# $BUILDSCRIPTS does not need to be accessed at all at run time
mkdir -p $ICDIR/modulefiles
cp -p $BUILDSCRIPTS/PrgEnv-fluidity $ICDIR/modulefiles
module use $ICDIR/modulefiles
...
export PATH=$PATH:$FLUIDITYDIR/bin:$ICDIR/bin
...
if ./configure --enable-2d-adaptivity >> configure-${TIMESTAMP}.log 2>&1; then
    job1=$(qsub -A $PROJECT $BUILDSCRIPTS/compile_icferst.pbs)
else
